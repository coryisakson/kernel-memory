...
public static class DocumentUploadTest
{
    ...
    public static async Task ItDownloadsPDFDocs(IKernelMemory memory, Action<string> log)
    {
        ...
        // Act
        var streamAbleFileInfo = await memory.ExportDocumentAsync(fileName, Id);
        var pdfBytes = (await streamAbleFileInfo.StreamAsync()).ReadAllBytes();
        log($"Export {streamAbleFileInfo.FileName}, size: {streamAbleFileInfo.Length}");

        // Assert
        var expected = ComputeChecksum(File.ReadAllBytes(fileName));
